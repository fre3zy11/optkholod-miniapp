...
## Автоперевод названий через Yandex Cloud

Перевод выполняется только на backend: API-ключ не попадает в браузер или Mini App.
Создайте сервисный аккаунт с ролью `ai.translate.user`, выпустите API-ключ со
scope `yc.ai.translate.execute` и добавьте на сервер в `.env`:

```text
YANDEX_TRANSLATE_API_KEY=ваш_секретный_api_ключ
# Необязательно для API-ключа сервисного аккаунта; укажите, если этого требует способ авторизации:
YANDEX_TRANSLATE_FOLDER_ID=идентификатор_каталога
```

После изменения `.env` перезапустите контейнер или процесс бота. Не добавляйте
настоящий ключ в Git, frontend-переменные `VITE_*` или скриншоты.
